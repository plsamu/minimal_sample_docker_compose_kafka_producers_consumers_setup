...
if [[ -z "$KAFKA_LISTENERS" ]]; then
  ...
  echo 'Set KAFKA_LISTENERS to edit the listeners'
else
...
if [[ -z "$KAFKA_ADVERTISED_LISTENERS" ]]; then
  ...
  echo 'Set KAFKA_ADVERTISED_LISTENERS to edit the advertised.listeners'
else
...
if [[ -z "$KAFKA_LISTENER_SECURITY_PROTOCOL_MAP" ]]; then
  ...
  echo 'Set KAFKA_LISTENER_SECURITY_PROTOCOL_MAP to edit the listener.security.protocol.map'
else
...
if [[ -z "$KAFKA_INTER_BROKER_LISTENER_NAME" ]]; then
  ...
  echo 'Set KAFKA_INTER_BROKER_LISTENER_NAME to edit the inter.broker.listener.name'  
else
